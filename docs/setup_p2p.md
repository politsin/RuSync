...
1. Install and enable RuSync in the intended Vault.
2. Open onboarding from the `Welcome to RuSync` Notice.
...
8. After saving the P2P profile, open `RuSync: P2P Sync : Open P2P Status` from the command palette. The P2P ribbon icon provides the same destination. Select `Open connection` if signalling is disconnected.
...
1. Run `RuSync: Copy settings as a new Setup URI` from the command palette.
...
1. Install and enable RuSync in a new or separately backed-up Vault.
