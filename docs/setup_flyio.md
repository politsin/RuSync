...
This is how to configure fly.io and CouchDB on it for RuSync.
...
> At some point, we started to experience problems related to our IBM Cloudant account. At the same time, RuSync started to improve its functionality, requiring CouchDB in a more natural state to use all its features.
>
...
Creating app in /home/vorotamoroz/dev/rusync/utils/flyio
...
Validating /home/vorotamoroz/dev/rusync/utils/flyio/fly.toml
Platform: machines
...
After the initial setup, CouchDB needs some more customisations to be used from RuSync. It can be configured in browsers or by HTTP-REST APIs.
...
#### 6. Use it from RuSync

Now the CouchDB is ready to use from RuSync. We can use `https://billowing-dawn-6619.fly.dev` in URI, `campanella` in `Username` and `dfusiuada9suy` in `Password` on RuSync. The `Database name` could be anything you want.
